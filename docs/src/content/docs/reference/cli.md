...
The only valid `--skip` step names are `intent`, `rebase`, `format`, `lint`, `test`, `metrics`, `document`, `review`, `push`, `pr`, and `ci`. Repository gate names are refused.
...
Use `no-mistakes rerun` only after that monitor is no longer running, such as a closed PR, aborted or superseded run, idle timeout, or exhausted CI auto-fix attempts; see [`no-mistakes rerun`](#no-mistakes-rerun) for the restart conditions.
...
A newer same-branch run whose custody was explicitly returned also supersedes an older terminal run's claim, but only when that older run's recorded head never moved past the head you submitted, so a recovered branch is never re-blocked by a run that preceded the recovery and held nothing pipeline-authored.
An older terminal run that did move its head keeps blocking until its own custody is recovered or proven contained.
...
When a detached preserved head is adopted, recovery also hands the gate branch to it with an atomic compare-and-swap against the submitted head before moving your worktree, so the run you are told to start next is not rejected as a non-fast-forward gate push; a concurrent gate move loses that swap and the recovery refuses with `blocked_recover_gate_race` instead.
Any gate-side ref a refusal could not drop is named in that refusal rather than left for you to discover, because it still pins objects in the gate.
...
`no-mistakes rerun` is one-way where it is offered: the new run is active, so recovery then refuses with `blocked_recover_run_active` until that run ends.
...
use rerun to bypass a gate. Never rerun while a gate awaits your response or a
step is actively working, unless you are deliberately discarding that run.
