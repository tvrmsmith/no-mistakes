...
- When `NO_MISTAKES_CHANGED_FILES` cannot carry every changed path, because the list exceeds its byte cap or a path contains a newline or a carriage return, the attempt reports a `warning` finding naming how many paths the variable dropped, so the omission reaches the outcome and the PR body. `NO_MISTAKES_CHANGED_FILE_COUNT` still carries the true total.
