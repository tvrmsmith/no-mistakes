...
- When the reviewer's final output is rejected, reruns a fresh, session-free review with the same prompt plus a note quoting the validation error, up to three attempts in total.
  ...
  Each of those attempts must satisfy the skill mandate above; exhausting the mandate fails the step immediately rather than spending a rerun.
...
- Whenever a repair revalidates - either because the setting requires it or because continuity cannot be proven - restarts at Format, the same boundary an agent-authored commit restarts to elsewhere in the pipeline: Intent and Rebase keep their results, steps already skipped for the run stay skipped, the run id is unchanged, and the durable auto-fix attempt count carries across. Earlier cycles remain in the run's round history; the step's own status shows the latest cycle. The fresh Review cycle does not inherit the superseded cycle's outstanding finding carry set
