...
- Every selected unit's command runs via the platform shell (`sh -c` on POSIX, `cmd.exe /c` on Windows), exactly once per attempt, and captures output. Non-zero exit produces an `error` finding and stops further unit execution for that attempt. Configure **targeted** commands (see repo-config); do not treat these as CI-parity complete-suite configuration.
...
- On a GitHub non-base branch with an existing PR raised through no-mistakes, rewrites the existing pipeline attestation for the exact commit about to be pushed before mutating the branch. The ordinary Push step uses this run's current step snapshot; it never adds an attestation to an unrelated PR. A write that cannot settle aborts before the push.
- If the GitHub SCM host is unavailable, including a missing CLI or failed authentication, skips that rewrite consistently with the PR step and continues the git push. Any stale attestation remains fail-closed because consumers require its `head_sha` to equal the PR head.
...
- If the repository cannot be named on its own provider (for example an unresolvable GitHub slug or Bitbucket repository reference), the forge is never asked, so the step pauses for user approval with an ask-user finding instead of skipping: an unread check list is unproven, not green. Cases where there is simply no provider integration to consult still skip the step
