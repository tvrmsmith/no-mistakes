...
- Runs the review through the `comprehensive-code-review` skill rather than an inline read of the diff. If the agent answers without invoking it, the turn is retried once in a fresh reviewer session and the step then fails; an inline review is never accepted, and there is no config opt-out
