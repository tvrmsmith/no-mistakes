...
- Runs the review through the `comprehensive-code-review` skill rather than an inline read of the diff. The step checks before its first turn that the skill is installed - in the repository or in your home directory, under `.claude/skills`, `.agents/skills`, or a Claude Code plugin's `skills` directory - and fails with an install message when it is not. If the agent answers without invoking it, the turn is retried once in a fresh reviewer session and the step then fails; an inline review is never accepted. A machine with no skill directory at all is exempt, since there is no absence to read
