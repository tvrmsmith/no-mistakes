...
The same exemption covers a run actively monitoring CI for an already-open PR (`lifecycle.ResumableCIMonitor`): a stop or restart preserves it too, and the next daemon start re-enters CI monitoring for the same PR.
...
- Resumes a run that was actively monitoring CI for an already-created PR, re-entering CI monitoring for the same PR, when its worktree and step history validate; only when recovery declines to resume such a run (adverse evidence, such as a missing worktree, or a crashed daemon's CI step mid-repair) is it ended as terminal `ci_monitor_interrupted`, with the run's error text naming the concrete reason. The PR stays open either way, so this is never reported as a pipeline failure
