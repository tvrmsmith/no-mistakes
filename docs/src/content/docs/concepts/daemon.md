...
The same exemption covers a run actively monitoring CI for an already-open PR: a stop or restart preserves it too, and the next daemon start re-enters CI monitoring for the same PR. A monitor carries two conditions a gate park does not, both checked before it is exempted: no auto-fix agent may still be attached to its CI step, and its worktree must hold no uncommitted work and sit at the head the run recorded. An auto-fix turn killed between its edits and its commit leaves exactly that state, and re-entering the monitor on top of it would let the next repair commit those leftovers under a message describing a different repair and push them to the open PR. A monitor that fails either check counts as blocking like any other active run.
...
`no-mistakes daemon stop` and `no-mistakes daemon restart` apply the same guard, including the same gate-parked and CI-monitor exemptions and the same step-plan check: if pending or running pipeline runs exist, each refuses by default and lists the active runs, and each takes its own `--force` to proceed anyway.
...
A run parked at a gate (`axi respond` waiting on you) is never waited on: stopping the daemon preserves it, so it stays parked in the database and the next daemon start resumes it, the same as it is today without `--drain`. Answer that gate while the drain is still waiting and the run rejoins the wait and the report, since it is working again. A run actively monitoring CI for an already-open PR gets the same treatment: it is exempt from the drain's wait, a clean stop preserves it, and the next daemon start re-enters CI monitoring for the same PR, since a drain that blocked on CI could run indefinitely. It is not reported under `Interrupted` and does not count as cut, because nothing was cut. A CI step partway through an auto-fix repair is not that case: an agent is working, so the run is waited on like any other and bounded by the deadline below. That holds whether the repair is recorded as a fix round or is still running inline with the agent's process attached to the step. The drain releases a monitor from its wait only when the coming stop will really preserve it, so a monitor whose worktree holds uncommitted work, or sits at a commit the run never recorded, is waited on like ordinary in-flight work rather than let out of the wait and then ended by the stop.
Both classifications are rechecked while the drain waits, so a run that parks at a gate or starts a CI monitor after the drain begins is released from the wait just as one that was already there.
...
On startup, the daemon checks for runs that were left in `pending` or `running` status, whether the previous daemon crashed while they were active or was stopped cleanly while one of them was parked at a gate or sitting in a live CI monitor:
...
- Resumes a run that was actively monitoring CI for an already-created PR, re-entering CI monitoring for the same PR, when its worktree and step history validate, which for a monitor includes a worktree holding no uncommitted work; only when recovery declines to resume such a run (adverse evidence, such as a missing worktree, leftovers from an interrupted auto-fix turn, or a crashed daemon's CI step mid-repair) is it ended as terminal `ci_monitor_interrupted`, with the run's error text naming the concrete reason. The PR stays open either way, so this is never reported as a pipeline failure
...
1. Cancels every active run, which fails a run interrupted mid-step but leaves a run parked at a gate, or one sitting in a live CI monitor, preserved for the next start
