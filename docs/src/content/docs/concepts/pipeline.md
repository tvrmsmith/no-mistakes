...
- Set explicit `commands.lint`, `commands.format`, an optional **targeted** `commands.test` (local intent validation only; not a full CI suite), and an optional `commands.metrics`, with `commands.prepare` when their dependencies must be materialized once in the isolated worktree.
