...
rebased, formatted, linted, tested, documented, reviewed, and safe to share.
...
- **Before** the code is public, it rebases, formats, runs lint, runs baseline tests, gathers user-facing test evidence when intent is available, checks that docs are in sync, runs a structured AI code review, and only then pushes to the configured target and opens the PR.
...
- A fixed, opinionated core pipeline: `intent → rebase → format → lint → test → metrics → document → review → push → pr → ci`. Repositories can add [command gates](/no-mistakes/reference/repo-config/#gates) after selected core steps, but cannot remove or reorder those steps.
