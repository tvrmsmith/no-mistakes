...
func TestAutoFixableFindings_FiltersToAutoFix(t *testing.T) {
	...
	fixable := AutoFixableFindings(f, FindingSeverityInfo)
...
func TestAutoFixableFindings_SeverityFloorExcludesLowerSeverities(t *testing.T) {
	f := Findings{
		Items: []Finding{
			{ID: "f1", Severity: FindingSeverityError, Description: "bug", Action: ActionAutoFix},
			{ID: "f2", Severity: FindingSeverityWarning, Description: "missing check", Action: ActionAutoFix},
			{ID: "f3", Severity: FindingSeverityInfo, Description: "nit", Action: ActionAutoFix},
		},
	}
	tests := []struct {
		floor string
		...
		{FindingSeverityInfo, []string{"f1", "f2", "f3"}},
		{"", []string{"f1", "f2", "f3"}},
		{FindingSeverityWarning, []string{"f1", "f2"}},
		{FindingSeverityError, []string{"f1"}},
	}
...
func TestAutoFixableFindings_UnknownSeverityPassesEverySeverityFloor(t *testing.T) {
	...
	for _, floor := range []string{FindingSeverityInfo, FindingSeverityWarning, FindingSeverityError, "nonsense"} {
...
func TestEmptyActionFindingFailsClosedToAskUser(t *testing.T) {
	...
	if len(AutoFixableFindings(f, FindingSeverityInfo).Items) != 0 {
