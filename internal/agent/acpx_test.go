...
func TestParseAcpxJSONEvents_PreservesUsagePresence(t *testing.T) {
	...
	var usage TokenUsage
	if _, _, err := parseAcpxJSONEvents(t.Context(), strings.NewReader(events), nil, &usage); err != nil {
		t.Fatal(err)
	}
...
func TestParseAcpxJSONEvents_CapturesFirstError(t *testing.T) {
	...
	var usage TokenUsage
	out, stdoutErr, err := parseAcpxJSONEvents(t.Context(), strings.NewReader(events), nil, &usage)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestParseAcpxJSONEvents_EmptyStream(t *testing.T) {
	var usage TokenUsage
	out, stdoutErr, err := parseAcpxJSONEvents(t.Context(), strings.NewReader(""), nil, &usage)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if out != "" {
...
`
	var usage TokenUsage
	out, _, err := parseAcpxJSONEvents(t.Context(), strings.NewReader(events), nil, &usage)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if out != "safe" {
