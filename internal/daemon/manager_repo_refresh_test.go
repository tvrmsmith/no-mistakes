...
func TestRunStartRefreshesCloneURLWithoutMutatingRemotes(t *testing.T) {
	...
	runID, err := manager.startRun(t.Context(), repo, "main", head, refreshTestZeroSHA, "test", nil, "refresh repository URL", "", false)
	if err != nil {
...
func TestRunStartURLRefreshFailuresWarnSafelyAndContinueWithOldRegistration(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			t.Cleanup(manager.Shutdown)
			runID, err := manager.startRun(t.Context(), before, "main", head, refreshTestZeroSHA, "test", nil, "refresh failure must fail open", "", false)
			if err != nil {
