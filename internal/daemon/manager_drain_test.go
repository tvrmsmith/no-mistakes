...
// TestShutdown_GateParkedRunSignalsThePreservingCause pins the premise the
// parked carve-out rests on: the drain leaves a parked run to the clean-stop
// path, and that path has to hand it the cause its executor preserves it by.
// Shutdown signals every run, parked included, with pipeline.ErrDaemonShutdown,
// which the gate wait translates into pipeline.ErrParkPreserved instead of
// failing the step and clearing awaiting_agent_since. Withholding the signal
// instead would leave the goroutine blocked at its gate forever. The end-to-end
// preservation is covered by internal/daemon/shutdown_park_test.go.
func TestShutdown_GateParkedRunSignalsThePreservingCause(t *testing.T) {
	m, database, repo := newDrainTestManager(t)
	...
	if cause := context.Cause(parkedCtx); !errors.Is(cause, pipeline.ErrDaemonShutdown) {
		t.Fatalf("cancel cause for the parked run = %v, want %v so its executor preserves it", cause, pipeline.ErrDaemonShutdown)
	}
	if cause := context.Cause(workingCtx); !errors.Is(cause, pipeline.ErrDaemonShutdown) {
		t.Fatalf("cancel cause for run %s = %v, want %v", working.ID, cause, pipeline.ErrDaemonShutdown)
	}
}
