...
func TestBuildHost_Forgejo(t *testing.T) {
	...
	host, skip := buildHost(sctx, scm.ProviderForgejo)
	if host == nil || skip.Reason != "" {
		t.Fatalf("buildHost() = (%v, %q), want Forgejo host", host, skip.Reason)
	}
...
func TestBuildHost_ForgejoRejectsForkRouting(t *testing.T) {
	...
	host, skip := buildHost(sctx, scm.ProviderForgejo)
	if host != nil || !strings.Contains(skip.Reason, "fork") {
		t.Fatalf("buildHost() = (%v, %q), want explicit fork rejection", host, skip.Reason)
	}
}
