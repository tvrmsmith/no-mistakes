...
//
// PERSONAL: the review prompt mandates the comprehensive-code-review skill, but
// prompt text is discretionary - measured drift had roughly half of review
// turns collapse into a single inline pass, silently skipping every per-aspect
// reviewer. Because that is a discretionary adherence miss and the turn is
// cheaply repeatable, retry it once before failing the step, which fails the
// whole run. The retry is another session-free turn, so it cannot inherit the
