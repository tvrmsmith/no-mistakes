...
func TestExecutor_RevalidationGateRemainsRecoverable(t *testing.T) {
	...
	// recovery, and startRun writes it before the executor runs.
...
// TestExecutor_CIStepFailingForOtherReasonStillFails pins that a CI step
// whose context is cancelled for any cause other than a clean daemon
// shutdown still lands failed, so the shutdown-preservation branch cannot
// widen beyond that one cause.
