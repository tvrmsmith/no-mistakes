...
func TestPiProfileHelpStatusAndUsageEvidence(t *testing.T) {
	...
	for _, view := range []runView{runViewFromDB(run, nil), runViewFromIPC(&ipc.RunInfo{ID: run.ID, PiProfile: pin})} {
	...
	var out bytes.Buffer
	if err := renderRunAgentPerf(newPrinter(&out), d, run.ID); err != nil {
		t.Fatal(err)
	}
