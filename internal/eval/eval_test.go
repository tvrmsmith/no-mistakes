...
func TestReplayRestoresCaseIntoAnIsolatedWorktree(t *testing.T) {
	...
	const reply = `{"type":"assistant","message":{"content":[{"type":"tool_use","name":"Skill","input":{"skill":"comprehensive-code-review"}}]}}
...
func TestReplayPinsCandidateModelAndEffortOnTheHarness(t *testing.T) {
	...
	fake := filepath.Join(fakeDir, "claude")
	const reply = `{"type":"assistant","message":{"content":[{"type":"tool_use","name":"Skill","input":{"skill":"comprehensive-code-review"}}]}}
{"type":"result","subtype":"success","is_error":false,"structured_output":{"findings":[],"risk_level":"low","risk_rationale":"clean","risk_scope":"source-or-external"},"usage":{"input_tokens":1,"output_tokens":1}}
`
	var script string
	if runtime.GOOS == "windows" {
		fake += ".cmd"
